...
NUMERIC_ID NUMBER;
BEGIN
 SELECT NUMERIC_ID INTO NUMERIC_ID FROM NUMERIC WHERE INSTANCE_ID = insert_update_numeric.INSTANCE_ID AND METRIC_ID = insert_update_numeric.METRIC_ID AND UNIQUE_DEVICE_IDENTIFIER = insert_update_numeric.UNIQUE_DEVICE_IDENTIFIER AND UNIT_ID = insert_update_numeric.UNIT_ID AND VENDOR_METRIC_ID = insert_update_numeric.VENDOR_METRIC_ID;
 EXCEPTION WHEN NO_DATA_FOUND THEN
 BEGIN
    SELECT NUMERIC_SEQ.nextval into NUMERIC_ID FROM dual;
    INSERT INTO NUMERIC (NUMERIC_ID, UNIQUE_DEVICE_IDENTIFIER, METRIC_ID, VENDOR_METRIC_ID, INSTANCE_ID, UNIT_ID) VALUES (NUMERIC_ID, UNIQUE_DEVICE_IDENTIFIER, METRIC_ID, VENDOR_METRIC_ID, INSTANCE_ID, UNIT_ID);
 END;
